...
const {createUser, getAllUsers, getUser, getUserById, getAllLinksByUser, getUserByUsername} = require('../db');      
...
usersRouter.get("/me", async(req, res, next) =>{
    ...
    try{
      if (!headersAuth) return res.status(403).send({ message: `Please login` });
        ...
        const all = await getAllUsers();// erase
...
usersRouter.get('/:username/links', async(req, res, next)=>{
    ...
    try {    
        ...
        const links = await getAllLinksByUser(user)
        links ? res.send(links) : res.status(403).send({ message: `This User has no Links shared.` })
    } catch (error) {
        next(error)
    }
})
